...
impl BaseNode for MoveNode {
  fn calculate_output(&mut self, agent: Rc<RefCell<Agent>>, world: &World) {
      ...
      //bounds check
      if agent.borrow().x > world.width as f64 {
          unsafe { (*agent.as_ptr()).x = world.width as f64 };
      }

      if agent.borrow().y > world.height as f64 {
          unsafe { (*agent.as_ptr()).y = world.height as f64 };
      }
